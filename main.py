...
@bot.on_message(filters.command("setupchat", prefixes=["/", ".", "?", "-"]) & ~filters.private)
async def addchat(_, message):
    ...
    if not is_kuki:
        ...
        await message.reply_text(f"Already Set Up Kuki Chatbot of this Group Is @{message.chat.username}")


@bot.on_message(filters.command("removechat", prefixes=["/", ".", "?", "-"]) & ~filters.private)
async def rmchat(_, message):
    ...
    is_kuki = kuki.find_one({"chat_id": message.chat.id})
    if not is_kuki:
        await message.reply_text("Kuki ChatBot is already disabled")
    else:
        kuki.delete_one({"chat_id": message.chat.id})
        await message.reply_text("✅ | Kuki Chatbot is disabled!")
...
async def kukiai(client: Client, message: Message):
    ...
    if is_kuki:
        if message.reply_to_message:
            ...
            await bot.send_message(ERROR_LOG, f"{error}")
...
@bot.on_message(filters.command("chat", prefixes=["/", ".", "?", "-"]))
async def kukiai(client: Client, message: Message):
    await bot.send_chat_action(message.chat.id, "typing")
    if not message.text:
        msg = "/"
...
async def start(client, message):
    ...
    if message.chat.type != "private":
        buttons = InlineKeyboardMarkup(
            [[InlineKeyboardButton(text="Click here", url=f"t.me/{busername}?start")]]
        )
        await message.reply("Contact me in PM", reply_markup=buttons)
    else:
        buttons = [
            [
...
async def help(client, message):
    ...
    if message.chat.type != "private":
        buttons = InlineKeyboardMarkup(
            [[InlineKeyboardButton(text="Click here", url=f"t.me/{busername}?start=help_")]]
        )
        await message.reply("Contact me in PM", reply_markup=buttons)
    else:
        await message.reply_text(
            "/start - Start The Bot\n/chat - Send a message to this bot\n/setupchat - Activate Kuki Chatbot In Group\n/removechat - Disable Kuki Chatbot In Group"
        )
